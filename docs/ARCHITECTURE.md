...
> - **Methodology**: the live methodology is the knowledge pipeline (`er-save-reader knowledge run` → claims store) plus the one reference resolver in `crates/wasm-event-flags` (ADR-0005), consumed by both the app and elden-map. See `docs/WASM-EVENT-FLAGS.md`, `CONTEXT.md`, `docs/adr/`.
