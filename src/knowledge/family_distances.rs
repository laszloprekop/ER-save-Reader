//! The argv adapters for the five origin/family `knowledge` commands. Each reads
//! its args, calls the shared measurement/alignment/constants in `origin_model`,
//! and emits one claims artifact through the `Claims` emitter (ADR-0004):
//!
//!   family-distances  — is the distance BETWEEN flag families constant?
//!   origin-probe      — is the remaining drift explained by a record count?
//!   list-hunt         — WHERE does the save grow between ga_end and the families?
//!   validate-origin   — do the origin constants hold on out-of-sample characters?
//!   family-constants  — each family's distance from the origin, from its flips.
//!
...
//! variable-length u32 record list sitting ahead of the flag data. These commands
//! confirmed the families are rigidly locked to each other (`family-distances`),
//! then pinned the single origin that positions all of them.
//!
//! METHOD (and its limits). The shared base search (`origin_model::measure_all`)
//! accepts a base only where it is the UNIQUE position in a bounded window at
//! which every flag whose state is known for that file reads as expected. Those
//! expectations come from set-monotonicity: a flag verified to flip in an earlier
//! capture of the same character is SET in every later one. The window is centred
//! on the family's already-measured range — these commands do NOT discover where a
//! family lives (the claims store already did that), they re-measure a known
//! family in files where no flag of that family flipped, so two families can be
//! observed in ONE file. A window centred on prior measurements cannot prove a
//! base from nothing, and this code does not claim to. Files with too few
//! constraining flags are reported rather than resolved, because a weakly
//! constrained window is exactly how the timeline re-annotation produced 32,893
//! impossible events (docs/BACKLOG.md step 3).
//!
//! Every output here is generated, never hand-edited (ADR-0004).
...
use super::origin_model::{
    measure_all, method_block, narrow, origin_constant, predict_base, read_json, scan_list_end,
    shift_at, write_claims, Measured, CLAIMS, MAX_SHIFT, ORIGIN_CONSTANTS, PROBE_SPAN, SCAN_STRIDE,
    SYNC_WINDOW,
};
use super::pipeline::{bit_of, family_rel, load_save, INPUT_ALLOCLISTS, INPUT_TRANSITIONS};
