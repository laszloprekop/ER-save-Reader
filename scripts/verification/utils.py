...
def detect_event_flags_start(slot_data: bytes) -> Optional[int]:
    ...
    it landed on lookalike regions (e.g. ~106,808 on backup slot 0 where the
    fixture-pinned base is 81,077).

    Returns None only if detection is not confident AND found no anchors.
    """
    from .ef_dump import detect_ef_offset_bytes
    entry = detect_ef_offset_bytes(slot_data)
    if entry["positive_score"] < 2:
        return None
    return int(entry["ef_offset"])
...
def detect_event_flags_start_robust(
    ...
    """DELEGATES to the Rust reference implementation (ADR-0005).

    The `hint_offset` parameter is accepted for backward compatibility but
    ignored: the gaEnd-windowed detection is deterministic per slot state.
    """
    _ = hint_offset
